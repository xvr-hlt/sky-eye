...
def pairwise_iou(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    ...
    return boxes_iou(boxes1_tensor, boxes2_tensor)
...
"""    
    ...
    for i in range(0, N, 32):
        area1 = boxes1[i:min(i+32, N)].area()

        wh = (rb[i:min(i+32, N), :] - lt[i:min(i+32, N), :]).clamp(min=0)  # [<=32,M,2]
        inter = wh[:, :, 0] * wh[:, :, 1]  # [<=32,M]

        # handle empty boxes
        iou[i:min(i+32, N), :] = torch.where(
            inter > 0,
